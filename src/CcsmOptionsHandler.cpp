...
static const std::set<std::pair<std::string, std::string>>
    metricAliasList(metricAliasListData,
                    metricAliasListData + sizeof metricAliasListData /
...
static llvm::cl::list<std::string, std::vector<std::string>> ExcludeFiles(
    "exclude-file", llvm::cl::desc("Exclude specified file from the metrics"),
    llvm::cl::CommaSeparated, llvm::cl::ZeroOrMore,
    llvm::cl::value_desc("regex"), llvm::cl::location(ExcludeFileList),
...
static llvm::cl::list<std::string, std::vector<std::string>>
    IncludeInParentFiles(
        "def-file",
        llvm::cl::desc(
            "Specifies files whose contents should not be individually counted "
...
static llvm::cl::opt<bool>
    UseShortNames("output-short-names",
...
static llvm::cl::opt<bool> ExcludeStandardHeaders(
    "exclude-std-headers", llvm::cl::desc("Exclude standard library headers"),
    llvm::cl::init(false), llvm::cl::cat(CCSMToolCategory));

static llvm::cl::opt<bool>
    DumpTokens("dump-tokens", llvm::cl::desc("Dump tokens as they are lexed"),
               llvm::cl::init(false), llvm::cl::cat(CCSMToolCategory));

static llvm::cl::opt<bool>
    DumpAST("dump-ast", llvm::cl::desc("Dump AST when it is processed"),
            llvm::cl::init(false), llvm::cl::cat(CCSMToolCategory));

static llvm::cl::opt<bool>
    DumpFnMap("dump-function-map",
              llvm::cl::desc("Dump the mapping of files/functions/locations"),
              llvm::cl::init(false), llvm::cl::cat(CCSMToolCategory));

static llvm::cl::opt<bool>
    NoGlobal("disable-global",
...
static llvm::cl::opt<bool> PrototypesAreFileScope(
    ...
    llvm::cl::desc(
        "The prototype part of a function declaration should be included in "
...
static llvm::cl::list<std::string, std::vector<std::string>> OutputOnlyMetrics(
    "output-metrics", llvm::cl::desc("Only output the specified metrics"),
    llvm::cl::CommaSeparated, llvm::cl::ZeroOrMore,
...
static llvm::cl::opt<MetricLimitsFormat_e> OutputLimitWarnings(
    ...
    llvm::cl::values(clEnumValN(METRIC_LIMITS_FORMAT_DEFAULT, "default",
                                "Default output format"),
                     clEnumValN(METRIC_LIMITS_FORMAT_GROUP_BY_FILE, "byfile",
                                "Group warnings by file"),
                     clEnumValN(METRIC_LIMITS_FORMAT_GCC, "gccbrief",
                                "Use GCC brief format")),
    llvm::cl::init(METRIC_LIMITS_FORMAT_DEFAULT),
    llvm::cl::cat(CCSMToolCategory));

static llvm::cl::list<std::string> LimitsFile(
    "limits", llvm::cl::desc("Specify a file to read limits data from"),
    llvm::cl::CommaSeparated, llvm::cl::ZeroOrMore,
    llvm::cl::value_desc("filename"), llvm::cl::cat(CCSMToolCategory));
...
void CcsmOptionsHandler::processOutputMetricList() {
    ...
    if (OutputMetricList.size() == 0) {
        ...
        for (std::vector<std::string>::const_iterator it =
             ...
             * regex*/
            for (uint16_t metric = 0; metric < METRIC_TYPE_MAX; metric++) {
                if (metricNameRegex.match(MetricUnit::getMetricShortName(
                        static_cast<MetricType_e>(metric)))) {
                    m_outputMetrics.insert(static_cast<MetricType_e>(metric));
                    added = true;
                }
            }
            ...
            for (std::set<std::pair<std::string, std::string>>::const_iterator
                     it = metricAliasList.begin();
                 (it != metricAliasList.end()); it++) {
                if (metricNameRegex.match(it->second)) {
                    m_outputMetrics.insert(
                        MetricUnit::getMetricByShortName(it->first));
                    added = true;
                }
            }

            if (!added) {
                std::cerr << "Unrecognised parameter to --output-metrics: "
                          << *it << "\n";
                exit(EXIT_FAILURE);
            }
        }
...
void CcsmOptionsHandler::ParseOptions(int argc, const char **const argv) {
    m_optionsParser =
        new clang::tooling::CommonOptionsParser(argc, argv, CCSMToolCategory);

    processOutputMetricList();

    m_metricOptions =
        new MetricOptions(&ExcludeFileList, &ExcludeFunctionList,
                          m_outputMetrics, &IncludeInParentFileList);
...
CcsmOptionsHandler::getOptionsParser() const {
...
const std::set<std::string> c11_std_headers = {
    sep + "assert.h" + end,    sep + "math.h" + end,
    sep + "stdlib.h" + end,    sep + "complex.h" + end,
    sep + "setjmp.h" + end,    sep + "stdnoreturn.h" + end,
    sep + "ctype.h" + end,     sep + "signal.h" + end,
    sep + "string.h" + end,    sep + "errno.h" + end,
    sep + "stdalign.h" + end,  sep + "tgmath.h" + end,
    sep + "fenv.h" + end,      sep + "stdarg.h" + end,
    sep + "threads.h" + end,   sep + "float.h" + end,
    sep + "stdatomic.h" + end, sep + "time.h" + end,
    sep + "inttypes.h" + end,  sep + "stdbool.h" + end,
    sep + "uchar.h" + end,     sep + "iso646.h" + end,
    sep + "stddef.h" + end,    sep + "wchar.h" + end,
    sep + "limits.h" + end,    sep + "stdint.h" + end,
    sep + "wctype.h" + end,    sep + "locale.h" + end,
    sep + "stdio.h" + end};

const std::set<std::string> cpp_std_headers = {
    sep + "algorithm" + end, sep + "fstream" + end, sep + "list" + end,
    sep + "regex" + end, sep + "tuple" + end, sep + "array" + end,
    sep + "functional" + end, sep + "locale" + end,
    sep + "scoped_allocator" + end, sep + "type_traits" + end,
    sep + "atomic" + end, sep + "future" + end, sep + "map" + end,
    sep + "set" + end, sep + "typeindex" + end, sep + "bitset" + end,
    sep + "initializer_list" + end, sep + "memory" + end, sep + "sstream" + end,
    sep + "typeinfo" + end, sep + "chrono" + end, sep + "iomanip" + end,
    sep + "mutex" + end, sep + "stack" + end, sep + "unordered_map" + end,
    sep + "codecvt" + end, sep + "ios" + end, sep + "new" + end,
    sep + "stdexcept" + end, sep + "unordered_set" + end, sep + "complex" + end,
    sep + "iosfwd" + end, sep + "numeric" + end, sep + "streambuf" + end,
    sep + "utility" + end, sep + "condition_variable" + end,
    sep + "iostream" + end, sep + "ostream" + end, sep + "string" + end,
    sep + "valarray" + end, sep + "deque" + end, sep + "istream" + end,
    sep + "queue" + end, sep + "strstream" + end, sep + "vector" + end,
    sep + "exception" + end, sep + "iterator" + end, sep + "random" + end,
    sep + "system_error" + end, sep + "forward_list" + end,
    sep + "limits" + end, sep + "ratio" + end, sep + "thread" + end,
    /* C++ headers for C library functions */
    sep + "cassert" + end, sep + "cinttypes" + end, sep + "csignal" + end,
    sep + "cstdio" + end, sep + "cwchar" + end, sep + "ccomplex" + end,
    sep + "ciso646" + end, sep + "cstdalign" + end, sep + "cstdlib" + end,
    sep + "cwctype" + end, sep + "cctype" + end, sep + "climits" + end,
    sep + "cstdarg" + end, sep + "cstring" + end, sep + "cerrno" + end,
    sep + "clocale" + end, sep + "cstdbool" + end, sep + "ctgmath" + end,
    sep + "cfenv" + end, sep + "cmath" + end, sep + "cstddef" + end,
    sep + "ctime" + end, sep + "cfloat" + end, sep + "csetjmp" + end,
...
void CcsmOptionsHandler::checkCompilerArgs(const char *const exeName) {
    ...
    if (m_usesCpp) {
        std::cerr
            << "WARNING: Proper support for C++ language constructs is not "
               "currently implemented";
    }

    if (m_metricOptions->getExcludeStdHeaders()) {
        if (m_usesCpp) {
            ExcludeFileList.insert(ExcludeFileList.end(),
                                   cpp_std_headers.begin(),
                                   cpp_std_headers.end());
        }

        if (m_usesC89) {
            ExcludeFileList.insert(ExcludeFileList.end(),
                                   c89_std_headers.begin(),
                                   c89_std_headers.end());
        }

        if (m_usesC99) {
            ExcludeFileList.insert(ExcludeFileList.end(),
                                   c99_std_headers.begin(),
                                   c99_std_headers.end());
        }

        if (m_usesC11) {
            ExcludeFileList.insert(ExcludeFileList.end(),
                                   c11_std_headers.begin(),
                                   c11_std_headers.end());
        }
    }
}
...
void CcsmOptionsHandler::analyseCompilerArgs(const char *const exeName) {
    std::string Path = llvm::sys::fs::getMainExecutable(
        exeName, (void *)(intptr_t)ccsm_marker);
