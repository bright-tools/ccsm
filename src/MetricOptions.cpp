...
bool MetricOptions::isFileInList(const std::vector<std::string> *const p_list,
...
bool MetricOptions::ShouldIncludeFile(const std::string &p_fn) {
    ...
    /* See if we already checked this file and if so, use that result.  If not,
       ...
       TODO: This caching mechanism will not take into account changes to
       ExcludeFiles */
...
bool MetricOptions::setOutputFile(const std::string p_fileName) {
    bool retVal = false;
    /* If the user specified a file to sent the output to, try and open it and
       set the output stream point to that handle */
