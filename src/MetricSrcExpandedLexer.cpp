...
const std::map<MetricType_e, MetricType_e>
    MetricSrcExpandedLexer::m_metricToBodyMetricMap(
        metricToBodyMetricMapData,
        metricToBodyMetricMapData + sizeof metricToBodyMetricMapData /
                                        sizeof metricToBodyMetricMapData[0]);

const static std::pair<clang::tok::TokenKind, MetricType_e>
...
#endif
        std::make_pair(clang::tok::exclaimequal,
                       METRIC_TYPE_OPERATOR_COMP_NOT_EQUAL),
        std::make_pair(clang::tok::percent,
                       METRIC_TYPE_OPERATOR_ARITHMETIC_MODULO),
        std::make_pair(clang::tok::percentequal,
...
void MetricSrcExpandedLexer::ProcessToken(clang::Token &p_token) {
    ...
    case clang::tok::eof:
        ...
        break;
    default:
        std::map<clang::tok::TokenKind, MetricType_e>::const_iterator
            typeLookup = m_tokenKindToTypeMap.find(p_token.getKind());

        if (typeLookup != m_tokenKindToTypeMap.end()) {
            m_currentUnit->increment((*typeLookup).second, fileAndLineLoc);
            if (m_inBody) {
                std::map<MetricType_e, MetricType_e>::const_iterator
                    bodyTypeLookup =
            ...
            if (p_token.isAnyIdentifier()) {
                std::string tok_data =
                    p_token.getIdentifierInfo()->getName().str();
                if (m_options.getDumpTokens()) {
                ...
                if (m_inBody) {
                    ...
                    m_currentUnit->increment(
                        METRIC_TYPE_BODY_UNRESERVED_IDENTIFIERS,
                        fileAndLineLoc);
                }
            } else {
                /* TODO */
...
void MetricSrcExpandedLexer::CloseOutFnOrMtd(void) {
    ...
    if (m_currentUnit != NULL) {
        ...
        m_currentUnit->setSupplementary(METRIC_TYPE_NUMERIC_CONSTANTS,
                                        m_currentFnNumerics);
        m_currentUnit->setSupplementary(METRIC_TYPE_STRING_LITERALS,
                                        m_currentFnStrings);
        m_currentUnit->setSupplementary(METRIC_TYPE_CHAR_CONSTS,
                                        m_currentFnCharConsts);
        m_currentUnit->setSupplementary(METRIC_TYPE_UNRESERVED_IDENTIFIERS,
...
void MetricSrcExpandedLexer::EnterFileScope(void) {
    m_currentFnNumerics =
        m_currentUnit->getSupplementary(METRIC_TYPE_NUMERIC_CONSTANTS);
    m_currentFnStrings =
        m_currentUnit->getSupplementary(METRIC_TYPE_STRING_LITERALS);
    m_currentFnCharConsts =
        m_currentUnit->getSupplementary(METRIC_TYPE_CHAR_CONSTS);
    m_currentFnIdentifiers =
        m_currentUnit->getSupplementary(METRIC_TYPE_UNRESERVED_IDENTIFIERS);
}