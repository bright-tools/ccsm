...
const std::set<MetricType_e> m_halsteadOperatorMetrics = {
    ...
    METRIC_TYPE_OPERATOR_BITWISE_NOT, METRIC_TYPE_OPERATOR_BITWISE_AND,
    METRIC_TYPE_OPERATOR_BITWISE_OR, METRIC_TYPE_OPERATOR_BITWISE_XOR,
    METRIC_TYPE_OPERATOR_SHIFT_LEFT, METRIC_TYPE_OPERATOR_SHIFT_RIGHT,
    ...
    //	METRIC_TYPE_INLINE,
    METRIC_TYPE_REGISTER, METRIC_TYPE_STATIC, METRIC_TYPE_TYPEDEF,
    //	METRIC_TYPE_VIRTUAL,
    //  METRIC_TYPE_MUTABLE,
    ...
    //	METRIC_TYPE_ASM,
    METRIC_TYPE_BREAK, METRIC_TYPE_CASE,
    // METRIC_TYPE_CLASS,
...
const std::set<MetricType_e> m_halsteadOperandMetrics = {
    METRIC_TYPE_BODY_CHAR,   METRIC_TYPE_BODY_DOUBLE,
    METRIC_TYPE_BODY_FLOAT,  METRIC_TYPE_BODY_INT,
    METRIC_TYPE_BODY_LONG,   METRIC_TYPE_BODY_SHORT,
    METRIC_TYPE_BODY_SIGNED, METRIC_TYPE_BODY_UNSIGNED,
    METRIC_TYPE_BODY_VOID
    // TODO
    // TODO - no _Bool, _Complex or _Imaginary
};

MetricUnit::counter_t MetricUnit::getCounter(const MetricType_e p_metricType,
...
MetricUnit *MetricUnit::getSubUnit(const std::string &p_name,
    ...
    /* For files, get the absolute path so that we can disambiguate between
     * files which have the same filename */
...
bool MetricUnit::isFnOrMethod(void) const {
    return ((m_type == METRIC_UNIT_FUNCTION) || (m_type == METRIC_UNIT_METHOD));
}
...
std::string MetricUnit::getUnitName(const bool p_useAbsFN,
    ...
    if ((this->m_type == METRIC_UNIT_FILE) && (!p_useAbsFN)) {
        ...
        if ((this->m_type == METRIC_UNIT_FUNCTION) && (p_usePrefix)) {
...
std::string MetricUnit::getScaledMetricString(const MetricType_e p_type,
                                              MetricUnit::counter_t p_val) {
...
MetricUnit::getFirstInstanceLocation(const MetricType_e p_type) const {
    ...
    /* For calculated metrics, map to an appropriate observed metric which
       ...
       has no location instances associated with it as it is calculated, but
       when reporting the instance location it makes sense to report the
